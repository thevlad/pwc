...
public class TikaHtmlParserWithMeta implements edu.uci.ics.crawler4j.parser.HtmlParser {

	protected static final Logger logger = LoggerFactory.getLogger(TikaHtmlParserWithMeta.class);
	...
	public TikaHtmlParserWithMeta(CrawlConfig config) throws InstantiationException, IllegalAccessException {
		this.config = config;
	...
	private Set<WebURL> getOutgoingUrls(String contextURL, HtmlContentHandler contentHandler, String contentCharset) {
		...
		for (ExtractedUrlAnchorPair urlAnchorPair : contentHandler.getOutgoingUrls()) {
			...
			if (!hrefLoweredCase.contains("javascript:") &&
				...
				Charset hrefCharset = ((contentCharset == null) || contentCharset.isEmpty()) ? StandardCharsets.UTF_8
